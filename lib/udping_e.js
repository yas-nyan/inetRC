...
const _env = {
    ...
    savepath: "./result/udping_result.txt",
    //何ミリ秒おきに送るか タイムアウト時間はその二倍
    wait: 1000,
    //走行ログデータを書き換えるパス
    logpath: "./result/inetRC_result.txt"
}
    ...
    constructor(env) {
        ...
        switch (this.env.mode) {
            case "client":
                ...
                break;
            case "server":
                this.server = new Server(this.env.host, this.env.port, this.env.wait , this.env.logpath);
...
class Server extends _Server {
    constructor(host, port, wait, logpath) {
        ...
        this.timeoutCheck = setInterval(this.TIMEOUT, this.wait, logpath);
    }
